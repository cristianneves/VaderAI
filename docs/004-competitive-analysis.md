...
outright: _"no question bank, no mock interview mode, no structured prep — it's
real-time only. Nothing exists between interviews."_ We generate a question set
...
|  2  | **No conversational memory**                                  | Every ask is a stateless single-turn call carrying only the raw transcript tail. The model cannot see its own prior answers, so it repeats itself and _"elaborate on that"_ is structurally impossible.                 |  M   |   9a    |
...
anti-proctoring evasion as _"permanently out of scope, not deferred."_ This
