 ...
 */
export const startPractice = async (token: string, sessionId: string): Promise<PracticeQuestion[]> =>
  ((await (await call(token, `/${sessionId}`, { method: 'POST' })).json()) as QuestionSet).questions;
